...
AT.prototype._init = function() {
  ...
  if (this.options.sendVerificationEmail && this.options.enforceEmailVerification) {
    Accounts.validateLoginAttempt(function(attempt) {
      ...
      if ((attempt.type !== 're-captcha' && attempt.type !== "password") || attempt.methodName !== "login") {
        return attempt.allowed;
      }
      ...
      var ok = true;
      var loginEmail = attempt.methodArguments[0].user && attempt.methodArguments[0].user.email.toLowerCase();
      if (loginEmail) {
