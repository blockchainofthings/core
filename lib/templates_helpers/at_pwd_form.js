...
AT.prototype.atPwdFormEvents = {
    ...
    "submit #at-pwd-form": function(event, t) {
        ...
        if (state === "signUp") {
            ...
            var hash = password !== undefined ? Accounts._hashPassword(password) : undefined;
            var options = {
                username: username,
                email: email,
                password: hash,
            ...
            // NOTE: for some reason that I do not comprehend, the updated
            //    value of the options variable cannot be accessed from the
            //    createUser() closure; it will always access its initial value
            //    instead. To work around this issue, we store the options
            //    variable in the template object, and retrieve it later from
            //    the createUser() closure instead of accessing the options
            ...
            if (preSignUpHook) {
              // Call hook and wait for its (asynchronous) response
              return preSignUpHook(password, options, (error) => {
                if (error) {
                  // Hook returned error. Report error and stop processing
                  AccountsTemplates.submitCallback(error, state);
                }
                else {
                  // Hook returned gracefully. Just proceed with processing
                  createUser();
                }
              });
            }
            else {
              // No hook. Just proceed with processing
              return createUser();
            }

            function createUser() {
              Meteor.call("ATCreateUserServer", t._signUp.options, function(error){
                  ...
                  AccountsTemplates.submitCallback(error, undefined, function(){
                      if (AccountsTemplates.options.sendVerificationEmail && AccountsTemplates.options.enforceEmailVerification){
                        ...
                        // No password set. Just finalize signup
                        AccountsTemplates.submitCallback(error, state, function () {
                          // Cleans up input fields' content
                          _.each(AccountsTemplates.getFields(), function(field){
                            // Considers only visible fields...
